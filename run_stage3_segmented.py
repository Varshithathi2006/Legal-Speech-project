...
# ──────────────────────────────────────────────────────────────────────
# Setup paths for Moot Court dataset
# ──────────────────────────────────────────────────────────────────────
...
LOG_FILE = os.path.join(FOLDERS["logs"], "stage3_segmented.log")
...
logger = logging.getLogger("stage3_segmented")
...
logger.info("Initializing Stage 3 Segmented Transcription...")
...
# 3. scope_flag != "True" (if scope_flag column exists)
# 4. transcription_status != "done"
candidate_mask = (metadata_df["status"] == "audio_extracted") & (metadata_df["transcription_status"] != "done")

if "scope_flag" in metadata_df.columns:
    candidate_mask = candidate_mask & (metadata_df["scope_flag"].astype(str) != "True")

candidates_df = metadata_df[candidate_mask].copy()

# Double check diarization JSON exists
valid_candidates = []
...
logger.info(f"Queued for Segmented ASR      : {len(valid_candidates)}")

print(f"Ready to process {len(valid_candidates)} videos.")
...
for counter, idx in enumerate(valid_candidates, 1):
    ...
    logger.info(f"\n[{counter}/{len(valid_candidates)}] Processing: {vid} | {case_name[:60]}")
    ...
    try:
        ...
        logger.info(f"DONE: {vid} | {len(transcribed_segments)} segments | {total_words} words")

    except Exception as e:
        error_msg = repr(e)
        logger.exception(f"FAILED transcription for {vid}: {error_msg}")
...
print(f"\nSTAGE 3 SEGMENTED TRANSCRIPTION COMPLETE! Transcribed files: {done_count}/{len(metadata_df)}")
